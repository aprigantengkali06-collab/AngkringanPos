...
// Inline SVG paths for nav icons
const icons: Record<string, string> = {
  ...
  toko:       '<path d="M2 8l2-5h12l2 5M2 8v1a3 3 0 0 0 6 0V8M8 9a3 3 0 0 0 6 0M14 9a3 3 0 0 0 6 0V8M2 18h16M7 18v-5h3v5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>',
  ...
  <div v-if="showShell" class="app-shell">
    ...
    <aside class="app-sidebar" :class="{ open: isDrawerOpen }">
      ...
      <div class="sidebar-brand">
        <div class="sidebar-logo">
          <!-- Minimalist skewer + bowl mark -->
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
            <rect x="9.5" y="1" width="3" height="11" rx="1.5" fill="white"/>
            <circle cx="11" cy="3.5" r="2.2" fill="white"/>
            <circle cx="11" cy="8"   r="1.9" fill="rgba(255,255,255,.6)"/>
            <circle cx="11" cy="12"  r="1.4" fill="rgba(255,255,255,.3)"/>
            <path d="M3.5 15.5Q11 12.5 18.5 15.5L17.5 20Q11 17.5 4.5 20Z" fill="white"/>
          </svg>
        </div>
        ...
        <button class="sidebar-close mobile-only" @click="closeDrawer" aria-label="Tutup menu">
      ...
      <nav class="sidebar-nav">
        ...
        <NuxtLink
          ...
          @click="closeDrawer"
        >
          <svg class="nav-item-icon" width="18" height="18" viewBox="0 0 20 20" fill="none" v-html="icons[item.icon]" />
          <span class="nav-item-label">{{ item.label }}</span>
        </NuxtLink>
      </nav>

      <!-- ── Footer (logout fix: safe-area padding, never cropped) ── -->
    ...
    <div class="app-main">
      <header class="topbar">
        <div class="topbar-left">
          <button class="hamburger mobile-only" @click="isDrawerOpen = true" aria-label="Buka menu">
            <span /><span /><span />
          </button>
    ...
    <nav class="mobile-bottom-nav mobile-only">
      <NuxtLink v-for="item in bottomNavItems" :key="item.to" :to="item.to" class="bottom-nav-item">
        <svg class="bottom-nav-svg" width="22" height="22" viewBox="0 0 20 20" fill="none" v-html="icons[item.icon]" />
